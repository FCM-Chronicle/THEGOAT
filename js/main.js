...
function advanceRound() {
    ...
    // 날짜 진행
    gameState.date.month = Math.min(5, 8 + Math.floor(currentRound / 4));
    ...
    const myFixture = roundFix.find(f => f.home === player.teamKey || f.away === player.teamKey);

    // 라운드 넘기기 + 훈련 리셋
    gameState.currentRound++;
    gameState.trainedThisRound = false;  // ← 경기 넘어갈 때 훈련 초기화
    updateTrainBtn();
    ...
    if (!myFixture) {
        ...
        renderSeasonMini();
        saveGame(gameState);
        showToast('이번 라운드는 쉬는 주입니다.', 'info');
    ...
    if (player.debuffs?.fatigue > 0) {
        player.rating = Math.max(50, player.rating - 5);
        player.debuffs.fatigue--;
        if (player.debuffs.fatigue === 0) player.rating += 5;
    }
    ...
    document.getElementById('match-log').style.display = 'block';

    // 매치 화면에도 동기화
    syncMatchScreen(result, myTeam, enemyTeam);
...
// 매치 화면 동기화
function syncMatchScreen(result, myTeam, enemyTeam) {
    renderMatchResult(result, myTeam, enemyTeam, 'match-result-detail');
    renderMatchLog(
        result.matchEvents.length > 0 ? result.matchEvents : result.log.map(t => ({ text: t })),
        'match-log-detail'
    );
    document.getElementById('match-log-detail').style.display = 'block';
}
...
function showTransferOfferModal(offer) {
    const leagueName = offer.teamLeague === 1 ? '1부' : offer.teamLeague === 2 ? '2부' : '3부';
    showEventModal({
        title: `📨 이적 오퍼 수신!`,
        desc: `${offer.teamName} (${leagueName} 리그)에서 공식 오퍼가 도착했습니다!\n\n이적료: ${offer.fee}억\n주급: ${offer.weeklyWage.toLocaleString()}만/주\n계약: ${offer.contractYears}년`,
        choices: [
            { label: '✅ 오퍼 수락', effects: {} },
            { label: '❌ 거절한다',  effects: {} },
        ]
...
function doTrainingRound() {
    ...
    if (gameState.trainedThisRound) {
        showToast('⛔ 이미 이번 경기 전에 훈련했습니다. 다음 경기 후 다시 가능합니다.', 'warning');
        return;
    }
...
function updateTrainBtn() {
    ...
    if (!btn) return;
    if (gameState?.trainedThisRound) {
        btn.textContent = '🏃 훈련 완료 (다음 경기 후 가능)';
        btn.disabled = true;
...
function handleSeasonEnd() {
    ...
    if (logEl) {
        logEl.innerHTML = result.events.map(e => `<div class="season-event">${e}</div>`).join('');
    }
...
function renderTransferTab() {
    ...
    if (infoEl) {
        ...
        const accessLeague = getAccessibleLeague(gameState.player.reputation);
        infoEl.innerHTML = `
            ...
            <div class="career-stat"><span>🎯 접근 가능</span><strong>${accessLeague}부 리그</strong></div>
            <div class="career-stat"><span>💵 현재 주급</span><strong>${gameState.player.weeklyWage.toLocaleString()}만</strong></div>
        `;
    }
    ...
    const leagueLabel = l => l === 1 ? '⭐1부' : l === 2 ? '🥈2부' : '🥉3부';
    listEl.innerHTML = '';
    targets.forEach(t => {
        ...
        card.innerHTML = `
            <div style="flex:1;">
                ...
                <span style="margin-left:8px;font-size:0.75rem;color:var(--text3);">${leagueLabel(t.league)}</span>
                <div class="transfer-info">팀 OVR ${t.avgOvr} · 예산 ${t.budget}억 ${t.canAfford ? '✅' : '⚠️예산부족'}</div>
            </div>
            <button class="sign-btn" data-key="${t.key}">지원</button>
        `;
...
function initTabs() {
    document.querySelectorAll('.tab-btn').forEach(btn => {
        btn.onclick = () => {
            ...
            if (tab === 'tab-stats'    && gameState) {
                renderStatBars(gameState.player, 'stat-bars');
                renderSPUpgradePanel(gameState.player, gameState.sp, 'sp-panel', spUpgrade);
            }
            if (tab === 'tab-career'   && gameState) {
                renderCareerCard(gameState.player, 'career-card');
                renderTendencies(gameState.player, 'tendencies');
                const el = document.getElementById('talent-display');
                if (el) {
                    const stars = '⭐'.repeat(gameState.player.hiddenTalent);
                    el.textContent = `${stars} 재능 ${gameState.player.hiddenTalent} (${
                        gameState.player.hiddenTalent === 1 ? '원툴 메타' :
                        gameState.player.hiddenTalent === 2 ? '월클 메타' : '신계 메타'
                    })`;
                }
            }
            if (tab === 'tab-league'   && gameState) {
                renderLeagueTable(
                    gameState.leagueTable,
                    gameState.teams[gameState.player.teamKey]?.league || 1,
                    'league-table', gameState.player.teamKey
            ...
            if (tab === 'tab-transfer' && gameState) renderTransferTab();
        };
    });
}
