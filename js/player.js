// ============================================================
// player.js v3 — 명성 시스템 개편 (OVR 기반), 재능 OVR 랜덤
// ============================================================
...
function calcStartingOvr(baseRating, hiddenTalent) {
    const pen = hiddenTalent === 3 ? Math.floor(Math.random() * 15)
              : hiddenTalent === 2 ? Math.floor(Math.random() * 8)
              : Math.floor(Math.random() * 3);
    return Math.max(55, baseRating - pen);
}

// ── 선수 생성 ─────────────────────────────────────────────────
function createPlayer({ name, position, country, age, rating, isIcon = false, isCustom = false }) {
    const hiddenTalent = Math.floor(Math.random() * 3) + 1;
...
function generateBaseStats(position, ovr) {
    ...
    for (const [stat, weight] of Object.entries(weights)) {
        ...
        stats[stat] = Math.min(Math.max(raw, 30), 105);
    }
    if (position === 'GK')
...
function calcInitialReputation(rating) {
    if (rating >= 92) return Math.floor(Math.random() * 100 + 800);
    if (rating >= 88) return Math.floor(Math.random() * 100 + 600);
    if (rating >= 84) return Math.floor(Math.random() * 100 + 400);
    if (rating >= 80) return Math.floor(Math.random() * 80  + 250);
    if (rating >= 75) return Math.floor(Math.random() * 60  + 120);
    if (rating >= 70) return Math.floor(Math.random() * 40  + 60);
    return Math.floor(Math.random() * 25 + 20);
}

// ── 핵심: OVR 기반 명성 증가 ────────────────────────────────
// 매 경기 후 호출. OVR이 높을수록 명성이 빠르게 오름.
function calcReputationGain(player, matchResult, opponentLeague) {
    const { win, draw, playerGoals, playerAssists, playerDefs, matchRating } = matchResult;

    // 1. OVR 기반 기본 명성 증가 (가장 큰 비중) — 지수적으로 증가
    // OVR 60→1, 70→4, 80→9, 90→16, 100→25, 110→36
    const ovrExcess = Math.max(0, player.rating - 60);
    const ovrBase = Math.round(Math.pow(ovrExcess / 10, 1.6));

    // 2. 경기 결과 보너스
    const resultBonus = win ? 3 : draw ? 1 : 0;

    // 3. 개인 활약 보너스
    const perfBonus = playerGoals * 4
                    + playerAssists * 2
                    + playerDefs * 1
                    + (matchRating >= 8.0 ? 5 : matchRating >= 7.0 ? 2 : 0);

    // 4. 상위 리그 상대 보너스
    const leagueBonus = opponentLeague === 1 ? 3 : opponentLeague === 2 ? 1 : 0;

    const total = ovrBase + resultBonus + perfBonus + leagueBonus;
...
function upgradeStatSP(player, statKey, amount = 1) {
    ...
    if (above130 >= player.hiddenTalent) {
        return { ok: false, reason: `재능(${player.hiddenTalent}) 한계! ${above130}개 스탯이 이미 130+` };
    }
...
function recalcOvr(player) {
    ...
    for (const [stat, w] of Object.entries(weights)) {
        if (player.stats[stat] !== undefined) { total += player.stats[stat] * w; wSum += w; }
    }
...
function checkTitles(player) {
    ...
    if ((stats.stamina||0)>=95)   titles.push({ id:'two_hearts', name:'❤️❤️ 2개의 심장' });
